...
type Props = {
  tags: string[]
  data: CollectionEntry<"contributions">[]
}

export default function Contributions({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [contributions, setContributions] = createSignal<CollectionEntry<"contributions">[]>([])

  createEffect(() => {
    setContributions(data.filter((entry) => 
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            SHOWING {contributions().length} OF {data.length} CONTRIBUTIONS
          </div>
          <ul class="flex flex-col gap-3">
            {contributions().map((contribution) => (
              <li>
                <ArrowCard entry={contribution} />
              </li>
            ))}
          </ul>
        </div>
